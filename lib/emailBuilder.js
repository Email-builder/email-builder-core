...
function EmailBuilder(options) {

  this.options  = options ? _.extend(EmailBuilder.Defaults, options) : EmailBuilder.Defaults;
...
EmailBuilder.prototype.prepareHtml = function(html) {
  ...
  linkTags.each(function(){
    ...
    if($this.attr('data-ignore')){
      ...
      var actualPath = path.join(dirname, href);
...
EmailBuilder.prototype.run = function(data, outputPath) {

  var file = Promise.resolve(data);
  ...
  .then(function(file){
    ...
    return this.inlineCss(file, outputPath)
    .then(this.writeFile)
